<script lang="ts">
	...
	import TrainConsoleCard from "./TrainConsoleCard.svelte";
	import TrainDiagnosticsCard from "./TrainDiagnosticsCard.svelte";
	import TrainFileBrowserModal from "./TrainFileBrowserModal.svelte";
	import TrainFitnessCard from "./TrainFitnessCard.svelte";
	import GaAnalyticsCard from "./GaAnalyticsCard.svelte";
	import TrainPageHeader from "./TrainPageHeader.svelte";
	import TrainSidebar from "./TrainSidebar.svelte";
	import { buildGaGenerationSeries, mergeGaLogRows } from "../gaAnalytics";
	import type {
		BuildProfile,
		ConsoleLine,
		DataMode,
	...
	let { paramsCollapsed = $bindable(false) }: Props = $props();
	...
	let gaRowsByKey = $state(new Map<string, GaLogPoint>());
	...
	const gaGenerationSeries = $derived.by<GaGenerationPoint[]>(() =>
		buildGaGenerationSeries(gaRowsByKey.values())
	);
	...
	const updateGaFromRows = (rows: Array<Record<string, unknown>>) => {
		gaRowsByKey = mergeGaLogRows(gaRowsByKey, rows);
	};

	const updateLogRows = (rows: Array<Record<string, unknown>>) => {
		if (logType === "ga") {
			updateGaFromRows(rows);
		} else {
	...
	const rebuildFitnessFromLogs = async (dir: string) => {
		...
		try {
			if (logType === "ga") {
				// The summary endpoint intentionally only returns fitness. Read the
				// existing candidate rows so train/eval PNL and gap diagnostics survive
				// a page reload or a run started with live updates disabled.
				let offset = 0;
				for (let page = 0; page < 10000; page += 1) {
					const payload = await fetchLogPayload(dir, offset);
					updateLogRows(payload.rows);
					if (
	...
	let previousTrainMode: TrainMode | null = null;

	const resetTrainingHeaderScroll = () => {
		if (typeof window === "undefined" || !window.matchMedia("(max-width: 1023px)").matches) return;
		window.requestAnimationFrame(() => {
			document.getElementById("training-page-header")?.scrollIntoView({ behavior: "auto", block: "start" });
		});
	};

	$effect(() => {
		const currentTrainMode = trainMode;
		if (previousTrainMode === null) {
			previousTrainMode = currentTrainMode;
			return;
		}
		if (currentTrainMode === previousTrainMode) return;
		previousTrainMode = currentTrainMode;
		resetTrainingHeaderScroll();
	});
</script>
...
<div class="min-h-screen bg-background">
	...
	<main class={`p-8 space-y-8 ${paramsCollapsed ? "lg:ml-[120px]" : "lg:ml-[360px]"}`}>
		...
		<div class="space-y-6">
			{#if trainMode === "ga"}
				<GaAnalyticsCard {training} series={gaGenerationSeries} />
			{:else}
				<TrainFitnessCard
					{training}
					{logLabel}
